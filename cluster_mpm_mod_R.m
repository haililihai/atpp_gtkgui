function mpm_cluster=cluster_mpm_mod_R(wd,prefix,part,sub_list,kc,prob_thre,mpm_thre)
...
vnii_ref=load_untouch_nii(strcat(wd,'/',sub{1},'/',prefix,'_',sub{1},'_',part,'_R_Sc/',part,'_R_',num2str(kc),'_MNI_relabel_group.nii'));
ref_img=vnii_ref.img;
IMGSIZE=size(ref_img);

prob_cluster=zeros([IMGSIZE,kc]);
for subi=1:sub_num
	sub_file=strcat(wd,'/',sub{subi},'/',prefix,'_',sub{subi},'_',part,'_R_Sc/',part,'_R_',num2str(kc),'_MNI_relabel_group.nii');
